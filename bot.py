...
@bot.tree.command(name="owner-add", description="Add a bot owner (Bot owner only)")
async def owner_add(interaction: discord.Interaction, username: str):
    if interaction.user.id != 931439270454001695:  # Only main bot owner
        await interaction.response.send_message("❌ You cannot use this command.", ephemeral=True)
        return

    # Ensure file exists
    if not os.path.exists("bot_owners.json"):
        with open("bot_owners.json", "w") as f:
            json.dump([], f)

    # Load existing owners
    with open("bot_owners.json", "r") as f:
        data = json.load(f)  # <- proper indentation!

    if username in data:
        await interaction.response.send_message("⚠️ This user is already a bot owner.", ephemeral=True)
        return

    data.append(username)

    with open("bot_owners.json", "w") as f:
        json.dump(data, f, indent=2)  # <- proper indentation!

    await interaction.response.send_message(f"✅ Added `{username}` as bot owner.", ephemeral=True)

# ---------------- OWNER REMOVE ----------------
@bot.tree.command(name="owner-rm", description="Remove a bot owner (Bot owner only)")
async def owner_rm(interaction: discord.Interaction, username: str):
    if interaction.user.id != 931439270454001695:  # Only main bot owner
        await interaction.response.send_message("❌ You cannot use this command.", ephemeral=True)
        return
    ...
    # Ensure file exists
    if not os.path.exists("bot_owners.json"):
        await interaction.response.send_message("❌ Bot owners database not found.", ephemeral=True)
        return
    ...
    if username not in data:
        await interaction.response.send_message(f"⚠️ User `{username}` is not a bot owner.", ephemeral=True)
        return
    ...
    await interaction.response.send_message(f"✅ Removed `{username}` from bot owners.", ephemeral=True)

# ---------------- OWNER LIST ----------------
@bot.tree.command(name="owner-list", description="Show all bot owners (Bot owner only)")
async def owner_list(interaction: discord.Interaction):
    ...
    if not data:
        await interaction.response.send_message("⚠️ No bot owners found.", ephemeral=True)
        return

    owners_text = "\n".join(f"- `{username}`" for username in data)
    embed = discord.Embed(
        title="🤖 Bot Owners",
        description=owners_text,
        color=discord.Color.blue()
    )
    await interaction.response.send_message(embed=embed, ephemeral=True)

@bot.tree.command(name="add-ip", description="Add IP to a VPS (Bot owner only)")
async def add_ip(interaction: Interaction, vps_name: str, ip: str):
    with open("bot_owners.json", "r") as f:
        data = json.load(f)
    ...
    if interaction.user.id not in data["owners"]:
        ...
        return  # <-- return yahan same indentation level par ho jahan if block khatam hota hai

    if not os.path.exists("vps_ips.json"):
        with open("vps_ips.json", "w") as f:
            json.dump({}, f)

    with open("vps_ips.json", "r") as f:
        vps_ips = json.load(f)

    vps_ips[vps_name] = ip

    with open("vps_ips.json", "w") as f:
        json.dump(vps_ips, f, indent=2)

    await interaction.response.send_message(f"✅ IP `{ip}` added to VPS `{vps_name}`.", ephemeral=True)


# ----------------------------
# /rm-ip - bot owner only
# ----------------------------
@bot.tree.command(name="rm-ip", description="Remove IP from a VPS (Bot owner only)")
async def rm_ip(interaction: Interaction, vps_name: str):
    with open("bot_owners.json", "r") as f:
        data = json.load(f)
    ...
    if not os.path.exists("vps_ips.json"):
        await interaction.response.send_message("❌ VPS IP database not found.", ephemeral=True)
        return

    with open("vps_ips.json", "r") as f:
        vps_ips = json.load(f)

    if vps_name not in vps_ips:
        await interaction.response.send_message("❌ VPS does not have an IP assigned.", ephemeral=True)
        return

    vps_ips.pop(vps_name)

    with open("vps_ips.json", "w") as f:
        json.dump(vps_ips, f, indent=2)

    await interaction.response.send_message(f"✅ IP removed from VPS `{vps_name}`.", ephemeral=True)

@bot.tree.command(name="ip-forward", description="Setup SSH port forwarding from your VPS (Owner only)")
async def ip_forward(interaction: discord.Interaction, vps_name: str, port: int):
    owner_id = str(interaction.user.id)

    # Load VPS owner database
    if not os.path.exists("vps_owners.json"):
        await interaction.response.send_message("❌ VPS database not found.", ephemeral=True)
        return
    ...
    if vps_name not in vps_ips:
        await interaction.response.send_message("❌ No IP assigned to this VPS. Use /add-ip first.", ephemeral=True)
        return
    ...
    try:
        # Run port forwarding **inside the VPS container**
        forward_cmd = (
            f"sshpass -p '2222' ssh -o StrictHostKeyChecking=no "
            f"-o UserKnownHostsFile=/dev/null "
            f"-N -R 0.0.0.0:{port}:localhost:22 root@{ip} &"
        )
        subprocess.getoutput(f"lxc exec {vps_name} -- bash -c \"{forward_cmd}\"")

        await interaction.followup.send(
            f"✅ Port `{port}` forwarded from VPS `{vps_name}` to `{ip}` successfully.",
            ephemeral=True
        )
    except Exception as e:
        await interaction.followup.send(f"⚠️ Failed to forward port:\n`{e}`", ephemeral=True)

@bot.tree.command(name="shelltmate", description="Start a host tmate SSH session (owner only)")
async def shelltmate(interaction: discord.Interaction):
    BOT_OWNER_ID = 931439270454001695  # apna ID daal lena
    ...
    try:
        # Kill any old tmate sockets (clean start)
        subprocess.run(["bash", "-c", "pkill -f 'tmate' || true"], check=False)
        subprocess.run(["bash", "-c", "rm -f /tmp/tmate.sock"], check=False)

        # Start a new tmate session safely
        subprocess.run(
            ["bash", "-c", "unset TMUX; tmate -S /tmp/tmate.sock new-session -d"],
            check=True
        )

        # Wait until ready
        subprocess.run(
            ["bash", "-c", "tmate -S /tmp/tmate.sock wait tmate-ready"],
            check=True
        )

        # Fetch SSH link only
        ssh_link = subprocess.getoutput("tmate -S /tmp/tmate.sock display -p '#{tmate_ssh}'").strip()

        if not ssh_link or "ssh" not in ssh_link:
            await interaction.followup.send("⚠️ Could not generate tmate SSH link.", ephemeral=True)
            return

        await interaction.followup.send(f"🔗 **Tmate SSH:** `{ssh_link}`", ephemeral=True)

    except subprocess.CalledProcessError as e:
        await interaction.followup.send(f"⚠️ Error creating tmate session:\n`{e}`", ephemeral=True)

@bot.tree.command(name="shell", description="Run a Linux command (bot owner only)")
async def shell(interaction: discord.Interaction, command: str):
    ...
    try:
        ...
        if len(output) > 1900:
            output = output[:1900] + "\n... (truncated)"
        embed = discord.Embed(
            title="🖥️ Shell Command Executed",
            description=f"**Command:** `{command}`\n\n```bash\n{output}\n```",
            color=discord.Color.dark_gray()
        )
        await interaction.followup.send(embed=embed, ephemeral=True)
    except Exception as e:
        await interaction.followup.send(f"⚠️ Error executing command:\n`{e}`", ephemeral=True)
